import React from "react";
import { Card, CardContent, Typography, Link, Box } from "@mui/material";
import WarningAmberIcon from "@mui/icons-material/WarningAmber";
import { darkTokens } from "./theme";

const StatusTile: React.FC = () => {
  return (
    <Box
      sx={{
        flex: 1, // takes remaining space
        overflowY: "auto",
        m: "10px 0",
        p: 1,
      }}
    >
      <Card
        sx={{
          border: `1px solid ${darkTokens.border.default}`,
          backgroundColor: darkTokens.background.surface,
          mb: 2,
          boxShadow: "none",
          borderRadius: "4px",
        }}
      >
        <CardContent>
          <img
            width="50"
            height="auto"
            alt="consent"
            src="data:image/svg+xml;base64,"
          ></img>
          <br />
          <Typography
            variant="body1"
            fontWeight={550}
            mb={1}
            color={darkTokens.text.primary}
          >
            Set up Leapwork Reports
          </Typography>

          <Typography variant="body2" color={darkTokens.text.secondary} mb={1}>
            We are almost there! Follow the instructions to complete the setup
            and unlock Leapwork Reports.
          </Typography>
        </CardContent>
      </Card>
      <Card
        sx={{
          border: `1px solid ${darkTokens.border.default}`,
          backgroundColor: darkTokens.background.surface,
          mb: 2,
          boxShadow: "none",
          borderRadius: "4px",
        }}
      >
        <CardContent>
          <WarningAmberIcon
            sx={{ color: darkTokens.status.warning, fontSize: 32, mr: 1 }}
          />
          <br />
          <Typography
            variant="body1"
            fontWeight={550}
            mb={1}
            color={darkTokens.text.primary}
          >
            Having issues with Microsoft Edge automation?
          </Typography>

          <Typography variant="body2" color={darkTokens.text.secondary} mb={1}>
            Visit this{" "}
            <Link
              href="#"
              underline="hover"
              sx={{ fontWeight: 500, color: darkTokens.status.successLight }}
            >
              link
            </Link>{" "}
            to read about how to fix it now.
          </Typography>

          <Typography variant="body2" color={darkTokens.text.secondary}>
            See a list of other known issues{" "}
            <Link
              href="#"
              underline="hover"
              sx={{ fontWeight: 500, color: darkTokens.status.successLight }}
            >
              here
            </Link>
            .
          </Typography>
        </CardContent>
      </Card>
    </Box>
  );
};

export default StatusTile;
